...
class AccountProfile(LoginRequiredMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if UsersAuth.objects.filter(user=self.request.user.pk).exists():
...
def auth_code(request):
    ...
    user.save()
    send_mail(
        subject=f'Доска объявлений MMORPG: подтверждение e-mail',
...
class UpdateProfile(LoginRequiredMixin, UpdateView):
    ...
    template_name = 'allauth/account/profile_update.html'
